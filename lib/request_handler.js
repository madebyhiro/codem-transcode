...
class RequestHandler {
  constructor(logger, storageBackend, totalAvailableSlots, processorBackends) {
    this._logger = logger
    this._storageBackend = storageBackend
    this._totalAvailableSlots = totalAvailableSlots
    this._processorBackends = processorBackends
  ...
  _finalizeJobStatus(job, success, message) {
    if (success) {
      ...
      let processor = this._processors.get(job.id)
      // TODO: Make this an attribute on the processorBackend, e.g. 'lastMessage'
      job.message = (processor.output && processor.output.length > 0 ? processor.output.trim().split("\n").pop() : message)
      job.setFailed()
    }
  ...
  _spawnJob(job) {
    try {
      let Processor = this._processorBackends.get('codem-ffmpeg') // TODO: make this flexible
      var jobProcessor = new Processor(job.options)
    } catch(err) {
      return Promise.reject(err)
    }
    ...
    jobProcessor.on('error',    (err)            => { this._logger.warn(err); this._exitHandler(job.id, -1, null) })
