...
class RequestHandler {
  ...
  _exitHandler(processor, jobID, code, signal) {
    ...
    this._storageBackend.get(jobID)
      .then(job => this._finalizeJobStatus(job, processor, success, message))
      .catch(err => this._logger.warn({context: {id: jobID, error: (err || 'unknown')}}, "Updating exit status failed."))
  }
  
  _finalizeJobStatus(job, processor, success, message) {
    if (success) {
      job.message = message
      job.progress = 1.0
      job.setSuccess()
    ...
    delete this._processors[job.id]
