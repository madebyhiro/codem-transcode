...
Job.create = function(opts, queue) {
	...
	job.prepare(function onPrepared(message) {
		...
		if (message == "hasOutputDir"){
			job.hasOutputDir = true;
		}
		if (job.hasInput && job.hasOutputDir){
			if( queue==null ){
				job.spawn();
			}
		}
...
Job.prototype.delete = function() {
	dbHandler.deleteJob(this.id,
	function(err, result) {
		if (err){
			throw ("Error while deleting job.");
		}
	});
};
...
Job.prototype.prepare = function(callback) {
	...
	mkdirp.mkdirp(destination_dir, mode,
	function(err) {
		if (err) {
			fs.stat(destination_dir,
			function(err, stats) {
				if (err) {
...
Job.prototype.spawn = function() {
	...
	if (this.hasInput && this.hasOutputDir && !this.the_process) {
		...
		the_process.stderr.on('data',
		function(data) {
			job.progressHandler(data)
		});
		the_process.on('exit',
		function(code) {
			job.finalize(code, tmpFile);
		});
...
Job.prototype.finalize = function(code, tmpFile) {
	var job = this;
	if (code === 0) {
		fs.rename(tmpFile, job.opts['destination_file'],
		function(err) {
			if (err) {
				util.log(err);
...
Job.prototype.extractProgress = function(text) {
	var re = new RegExp(/time=(\d+):(\d+):(\d+)\.(\d+)/);
	var m = re.exec(text);
	...
	if (m != null) {
		...
		this.progress = Math.round((current / this.duration)*100);
	}
};
