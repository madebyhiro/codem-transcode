...
var Job = JobUtils.getDatabase().define('Job', {
    ...
    classMethods: {
        ...
        create: function(opts, queue, callback, completeCallback) {
    ...
    instanceMethods: {
        ...
        spawn: function() {
            ...
            if (this.hasInput && this.hasOutputDir && !this.the_process) {
                ...
                var encoderFromJobOpts = null;
                if(typeof this.parsedOpts()['encoder'] !== 'undefined') {
                    encoderFromJobOpts = this.parsedOpts()['encoder'];
                }

                var the_process = child_process.spawn(encoderFromJobOpts || config['encoder'], args);
        ...
        finalize: function(code, thumbnails) {
            var job = this;
            if (thumbnails)
                job.thumbnails = JSON.stringify(thumbnails);
