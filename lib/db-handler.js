...
exports.prepareDatabase = function(dbfile, callback) {
  if (db == null) this.getDatabase(dbfile);
...
exports.insertJob = function(job, callback) {
  db.run("INSERT INTO jobs VALUES (?,?,?,?,?,?,?,strftime('%s', 'now'),strftime('%s', 'now'))",
    [job.id, job.status, job.progress, job.duration, job.filesize, JSON.stringify(job.opts), job.message],
    callback
  );
}
...
exports.updateJob = function(job, callback) {
  db.run(
    "UPDATE jobs SET status=?, progress=?, duration=?, filesize=?, message=?, updated_at=strftime('%s', 'now') WHERE id=?",
    [job.status, job.progress, job.duration, job.filesize, job.message, job.id],
    callback
  );
}

exports.getJob = function(id, callback) {
  db.get("SELECT * FROM jobs WHERE id=? LIMIT 1", [id], callback);
}
