...
const NO_JOB_TO_UPDATE = "The storage backend does not contain the specified job to update."
...
class MemoryStorageBackend extends StorageBackend {
  ...
  update(job) {
    if (!this._contains(job)) return Promise.reject(new Error(NO_JOB_TO_UPDATE))
