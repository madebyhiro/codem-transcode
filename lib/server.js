...
class Server {
  constructor(port, address, logger, storageBackend, slots, processorBackends) {
    this._logger = logger
    this._requestHandler = new RequestHandler(logger, storageBackend, slots, processorBackends)
    this._createServer(port, address)
    this._addRoutes()
    this._logger.info({context: {address: address, port: port}}, "Started server.")
  }
